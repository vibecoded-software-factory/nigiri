...
extension TilingEngine {
    ...
    func animationCurve(named name: String) -> AnimationCurve {
        ...
        // niri's per-channel defaults (animations.rs): window-open is a
        // 150ms ease-out-expo fade/scale; everything else defaults to the
        // damping-ratio-1 stiffness-800 spring.
        if name == "window-open" { return .easing(Easing(durationMs: 150, curve: .easeOutExpo)) }
    ...
    private func scaled(_ curve: AnimationCurve) -> AnimationCurve {
        ...
        case .spring(let s):
            ...
            return .spring(
                Spring(
                    stiffness: s.omega0 * s.omega0 / (animationSlowdown * animationSlowdown),
    ...
    func animateFrames(
        _ targets: [(window: ManagedWindow, frame: CGRect)], animation: String = "window-movement",
        trackRing: Bool = true, verifyOnSettle: Bool = true,
        onWindowSettled: ((ManagedWindow) -> Void)? = nil, onRawSettle: (() -> Void)? = nil,
        ...
        let sameTargets =
            ...
            && zip(frameAnimationTargets, targets).allSatisfy {
                $0.0 === $1.window && ColumnLayoutEngine.isClose($0.1, $1.frame, tolerance: 0.5)
            }
        if sameTargets {
                    ...
                    do {
                        ...
                        if a.translationOnly {
                            try WindowMover.setPosition(
                                a.window.axElement, to: frame.origin,
                                assumeSettable: a.window.positionSettable)
                        } else {
                            try WindowMover.setFrame(a.window.axElement, to: frame)
                        }
                        a.window.axWriteLatencyEMA =
                            a.window.axWriteLatencyEMA * 0.8 + (CACurrentMediaTime() - writeStart) * 0.2
