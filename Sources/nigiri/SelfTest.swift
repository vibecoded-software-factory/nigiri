...
enum SelfTest {
    ...
    static func run() -> Never {
        ...
        // already on screen.
        let two = ColumnLayoutEngine.columnPlacements(columns: [column(0.5), column(0.5)],
                                                     usableWidth: usableWidth, maximizedIndex: nil)
        let four = ColumnLayoutEngine.columnPlacements(columns: (0..<4).map { _ in column(0.5) },
                                                      usableWidth: usableWidth, maximizedIndex: nil)
        ...
        let thirds = ColumnLayoutEngine.columnPlacements(columns: (0..<3).map { _ in column(1.0 / 3) },
                                                        usableWidth: usableWidth, maximizedIndex: nil)
        expectEqual(thirds.reduce(0) { $0 + $1.width } + 2 * ColumnLayoutEngine.gap, usableWidth,
                    "tres columnas de 1/3 llenan el area util exacto")

        let floored = ColumnLayoutEngine.columnPlacements(columns: [column(0.5, minWidth: 800), column(0.5)],
                                                         usableWidth: usableWidth, maximizedIndex: nil)
        ...
        let maxed = ColumnLayoutEngine.columnPlacements(columns: [column(0.5), column(0.5)],
        ...
        for px in [CGFloat(200), 720, 1000, usableWidth] {
            ...
            expectEqual(ColumnLayoutEngine.width(forProportion: p, usableWidth: usableWidth), px,
        ...
        expectEqual(ColumnLayoutEngine.grantedX(1470, width: 720, screenFrame: screen), 1469, "borde derecho: 1px adentro")
        expectEqual(ColumnLayoutEngine.grantedX(1500, width: 720, screenFrame: screen), 1469, "mas alla del borde tambien")
        expectEqual(ColumnLayoutEngine.grantedX(-720, width: 720, screenFrame: screen), -719, "borde izquierdo: 1px adentro")
        expectEqual(ColumnLayoutEngine.grantedX(10, width: 720, screenFrame: screen), 10, "una x visible se respeta")

        // REGRESSION: with four columns, the ones out of view overlapped the
        // ones on screen.
        for offset in [CGFloat(0), 730, 1460] {
            let frames = ColumnLayoutEngine.targetFrames(columns: (0..<4).map { _ in column(0.5) },
                                                         in: screen, maximizedIndex: nil, viewOffset: offset)
            let onScreen = frames.map(\.frame).filter { min(screen.maxX, $0.maxX) - max(screen.minX, $0.minX) > 2 }
            for (i, a) in onScreen.enumerated() {
                for b in onScreen[(i + 1)...] {
                    expect(min(a.maxX, b.maxX) - max(a.minX, b.minX) <= 0.001,
        ...
        expectEqual(ColumnLayoutEngine.scrollOffset(toShow: 1, placements: two, currentOffset: 0, usableWidth: usableWidth),
                    0, "una columna ya visible no scrollea")
        let three = ColumnLayoutEngine.columnPlacements(columns: (0..<3).map { _ in column(0.5) },
                                                        usableWidth: usableWidth, maximizedIndex: nil)
        expectEqual(ColumnLayoutEngine.scrollOffset(toShow: 2, placements: three, currentOffset: 0, usableWidth: usableWidth),
                    three[2].x + three[2].width - usableWidth, "clipeada a la derecha: alinea ese borde")
        expectEqual(ColumnLayoutEngine.scrollOffset(toShow: 0, placements: three, currentOffset: 900, usableWidth: usableWidth),
                    three[0].x, "clipeada a la izquierda: alinea ese borde")
        expectEqual(ColumnLayoutEngine.scrollOffset(toShow: 9, placements: three, currentOffset: 123, usableWidth: usableWidth),
                    123, "un indice fuera de rango deja el offset como esta")

        // --- height split -------------------------------------------------
        let equalStack = (0..<3).map { _ in window() }
        expectEqual(ColumnLayoutEngine.naiveHeights(for: equalStack, usableHeight: 900)[0], 300, "tres Auto se reparten igual")
        ...
        expect(ColumnLayoutEngine.naiveHeights(for: overflowing, usableHeight: 900)[1] >= 0,
        ...
        if case .setProportion(let v)? = SizeChange.parse("50%") { expectEqual(v, 50, "50% fija") }
        else { failures.append("50% deberia ser setProportion") }
        if case .adjustProportion(let v)? = SizeChange.parse("+10%") { expectEqual(v, 10, "+10% ajusta") }
        else { failures.append("+10% deberia ser adjustProportion") }
        if case .setFixed(let v)? = SizeChange.parse("1000") { expectEqual(v, 1000, "1000 fija pixeles") }
        else { failures.append("1000 deberia ser setFixed") }
        if case .adjustFixed(let v)? = SizeChange.parse("-100") { expectEqual(v, -100, "-100 ajusta pixeles") }
        else { failures.append("-100 deberia ser adjustFixed") }
        ...
        expectEqual(Int(NigiriConfig.parseCombo("Mod+F")?.0 ?? 0), 0x20, "el combo usa el layout activo, no la posicion US")
        expect(NigiriConfig.parseCombo("Mod+F")?.1 == [.command, .option], "Mod es Cmd+Opt")
        NigiriConfig.layoutKeyCodes = [:]
        expectEqual(Int(NigiriConfig.parseCombo("Mod+Left")?.0 ?? 0), 0x7B, "una tecla sin caracter cae a la tabla fija")
        expect(NigiriConfig.parseCombo("Hyper+F19")?.1 == [.command, .option, .control, .shift], "Hyper son los cuatro")
        expect(NigiriConfig.parseCombo("Garbage+A") == nil, "un modificador inexistente se rechaza")
        expectEqual(NigiriConfig.wheelBindingKey(for: "Mod+WheelScrollDown") ?? "", "mod-down", "los binds de rueda de niri se aceptan")
        expectEqual(NigiriConfig.wheelBindingKey(for: "Mod+Ctrl+WheelScrollLeft") ?? "", "mod-ctrl-left", "con modificadores extra")
        expect(NigiriConfig.wheelBindingKey(for: "Mod+T") == nil, "un bind normal no es de rueda")

        // --- config tokenizer ----------------------------------------------
        expect(NigiriConfig.tokenize("spawn open -a \"Google Chrome\"") == ["spawn", "open", "-a", "Google Chrome"],
               "un token entre comillas conserva sus espacios")
        expect(NigiriConfig.tokenize("width 4 // comentario") == ["width", "4"], "los comentarios se descartan")
        expect(NigiriConfig.tokenize("title \"a // b\"") == ["title", "a // b"], "pero no adentro de comillas")
        ...
        expect(TilingEngine.spawnArgv("open -a 'Google Chrome'") == ["open", "-a", "Google Chrome"],
        ...
        var previous = 1.0
        var monotonic = true, nonNegative = true
        for step in 1...1000 {
        ...
        expect(TilingEngine.decorationIsVisible(CGRect(x: 10, y: 44, width: 720, height: 902), on: screen),
               "una ventana visible lleva decoracion")
        expect(!TilingEngine.decorationIsVisible(CGRect(x: 1469, y: 44, width: 720, height: 902), on: screen),
               "una estacionada a la derecha no")
        expect(!TilingEngine.decorationIsVisible(CGRect(x: -719, y: 44, width: 720, height: 902), on: screen),
        ...
        for count in [1, 2, 4, 6] {
            ...
            let p = ColumnLayoutEngine.columnPlacements(columns: cols, usableWidth: usableWidth, maximizedIndex: nil)
        ...
        expectEqual(ColumnLayoutEngine.width(forProportion: pFor800, usableWidth: usableWidth), 800,
        ...
        tabbed.focus(row: 1)
        let tabFrames = ColumnLayoutEngine.targetFrames(columns: [tabbed], in: screen, maximizedIndex: nil, viewOffset: 0)
        ...
        expectEqual(visible.first?.frame.height ?? 0, screen.height - 2 * ColumnLayoutEngine.gap,
        ...
        let geometries = ColumnLayoutEngine.columnGeometry(columns: mixed, in: screen, maximizedIndex: nil, viewOffset: 300)
        let mixedFrames = ColumnLayoutEngine.targetFrames(columns: mixed, in: screen, maximizedIndex: nil, viewOffset: 300)
        ...
        for step in 0...500 where critical.remainingFraction(at: Double(step) / 500) < -1e-9 { everNegative = true }
        expect(!everNegative, "criticamente amortiguado no sobrepasa")

        let underdamped = Spring(stiffness: 1100, dampingRatio: 0.6)
        var overshot = false
        for step in 0...500 where underdamped.remainingFraction(at: Double(step) / 500) < -1e-6 { overshot = true }
        ...
        for step in 0...500 where overdamped.remainingFraction(at: Double(step) / 500) < -1e-9 { overdampedNegative = true }
        expect(!overdampedNegative, "sobreamortiguado tampoco sobrepasa")
        expect(overdamped.remainingFraction(at: 0.1) > critical.remainingFraction(at: 0.1),
        ...
        expect(Easing(durationMs: 500, curve: .easeOutCubic).remainingFraction(at: 0.25)
        ...
        expect(Regex("Picture-in-Picture").matches("Firefox Picture-in-Picture"), "sin anclar, alcanza con encontrarlo")
        expect(!Regex("[").matches("cualquier cosa"), "un patron invalido no matchea nada (y avisa)")

        var m = NigiriConfig.Matcher()
        expect(m.matches(app: "Chrome", bundleID: "com.google.Chrome", title: "x",
        ...
        expect(m.matches(app: "Google Chrome", bundleID: "com.google.Chrome", title: "x",
                         isActive: false, isFloating: false, atStartup: false),
               "app-id resuelve contra el bundle id, que es el equivalente macOS")
        expect(!m.matches(app: "Alacritty", bundleID: "org.alacritty", title: "x",
                          isActive: false, isFloating: false, atStartup: false),
               "y no matchea otra app")
        m = NigiriConfig.Matcher()
        m.isActive = true
        expect(m.matches(app: "a", bundleID: nil, title: "t", isActive: true, isFloating: false, atStartup: false),
               "is-active=true matchea la enfocada")
        expect(!m.matches(app: "a", bundleID: nil, title: "t", isActive: false, isFloating: false, atStartup: false),
        ...
        expect(ColumnLayoutEngine.columnPlacements(columns: [column(0.5)], usableWidth: -20, maximizedIndex: nil)
        ...
        let dropCols = [CGRect(x: 0, y: 0, width: 300, height: 800),
        ...
        expect(TilingEngine.hoveredColumn(dropCols, x: 1200) == nil,
        ...
        // escritorio solo".
        let plan = TilingEngine.compactPlan([(empty: false, named: false),
        ...
        let named = TilingEngine.compactPlan([(empty: true, named: true),
                                             (empty: false, named: false)],
        ...
        expectEqual(TilingEngine.purgeVerdict(scans: 2, verdict: .alive).scans, 0, "aunque venga de dos ausencias")
        expect(TilingEngine.purgeVerdict(scans: 0, verdict: .dead).dead, "un proceso muerto no necesita contador")
        expect(!TilingEngine.purgeVerdict(scans: 0, verdict: .absentFromList).dead, "una ausencia sola no alcanza")
        expect(!TilingEngine.purgeVerdict(scans: 1, verdict: .absentFromList).dead, "dos tampoco")
        expect(TilingEngine.purgeVerdict(scans: 2, verdict: .absentFromList).dead, "la tercera consecutiva si")
        ...
        expect(ColumnLayoutEngine.insertPosition(columnFrames: tabbedFrames, point: overTab,
        ...
        expect(ColumnLayoutEngine.insertPosition(columnFrames: tabbedFrames, point: aboveTab,
        ...
        let prevLeft = column(0.5), prevRight = column(0.5)
        ...
        let p1 = column(0.5), p2 = column(0.5), p3 = column(0.5)
        ...
        expectEqual(ColumnLayoutEngine.presetIndex(after: 0.5, in: presetList, delta: 1, from: nil) ?? -1, 2,
                    "desde 0.5 hacia adelante toca 2/3, no 1/3")
        expectEqual(ColumnLayoutEngine.presetIndex(after: 0.5, in: presetList, delta: -1, from: nil) ?? -1, 0,
                    "y hacia atras, 1/3")
        expectEqual(ColumnLayoutEngine.presetIndex(after: 0.9, in: presetList, delta: 1, from: nil) ?? -1, 0,
                    "mas ancha que todos: envuelve al primero")
        expectEqual(ColumnLayoutEngine.presetIndex(after: 0.1, in: presetList, delta: -1, from: nil) ?? -1, 2,
                    "mas angosta que todos hacia atras: envuelve al ultimo")
        expectEqual(ColumnLayoutEngine.presetIndex(after: 0.5, in: presetList, delta: 1, from: 0) ?? -1, 1,
        ...
        expect(TilingEngine.isDialogLike(title: "Screen Recording", hasDefaultButton: true, hasCancelButton: false),
               "el prompt de permisos del sistema se adopta")
        expect(TilingEngine.isDialogLike(title: "", hasDefaultButton: true, hasCancelButton: false),
               "y uno sin titulo pero con boton de confirmar, tambien")
        expect(TilingEngine.isDialogLike(title: "Instalar", hasDefaultButton: false, hasCancelButton: false),
               "y uno con titulo y sin botones, tambien")
        expect(!TilingEngine.isDialogLike(title: "", hasDefaultButton: false, hasCancelButton: false),
        ...
        let effective = max(ColumnLayoutEngine.width(forProportion: restingProportion, usableWidth: tradeUsable), floorPx)
        expectEqual(effective, floorPx, "una columna apoyada en su piso mide el piso, no su proporcion")
        let ignoringFloor = ColumnLayoutEngine.width(forProportion: restingProportion, usableWidth: tradeUsable)
        expect(effective - ignoringFloor > 50,
        ...
        expectEqual(NigiriConfig.mouseBindingKey(for: "Shift+Mod+MouseMiddle") ?? "", "mod-shift-middle",
                    "el orden escrito no cambia la clave")
        expectEqual(NigiriConfig.mouseBindingKey(for: "Mod+Shift+MouseMiddle") ?? "",
                    NigiriConfig.mouseBindingKey(for: "Shift+Mod+MouseMiddle") ?? "x",
                    "las dos formas dan la misma clave")
        expectEqual(NigiriConfig.wheelBindingKey(for: "Mod+Shift+Ctrl+WheelScrollDown") ?? "", "mod-ctrl-shift-down",
                    "y la rueda usa el mismo orden canonico que el tap")
        expectEqual(NigiriConfig.bindingKey(mods: ["shift", "ctrl", "mod"], suffix: "down"), "mod-ctrl-shift-down",
                    "orden fijo: mod, cmd, opt, ctrl, shift")
        expectEqual(NigiriConfig.wheelBindingKey(for: "WheelScrollUp") ?? "", "mod-up",
        ...
        """)
        expectEqual(wheelCfg.wheelBindings["mod-shift-down"] ?? "", "focus-column-right",
                    "el orden escrito a mano no cambia la clave")
        expectEqual(wheelCfg.wheelBindings["mod-ctrl-up"] ?? "", "move-column-to-workspace-up",
        ...
        let offscreen = TilingEngine.decoratedFrames([
            .init(frame: CGRect(x: 999, y: 20, width: 300, height: 700), minimized: false, isFloating: false),
        ], screen: decoScreen)
        ...
        expectEqual(FullscreenStash.homeToRecord(isFloating: true, existingHome: nil, currentFrame: home) ?? .zero,
        ...
        expect(FullscreenStash.homeToRecord(isFloating: true, existingHome: home, currentFrame: parkedSpot) == nil,
               "y la segunda vez NO se reescribe: si no, el hogar pasa a ser el parkeo")
        expect(FullscreenStash.homeToRecord(isFloating: false, existingHome: nil, currentFrame: home) == nil,
        ...
        expectEqual(stashed.fullscreenHome ?? .zero, home, "el cambio de workspace ya no pisa el hogar del fullscreen")
        ...
        let placedAfterEpoch = ColumnLayoutEngine.columnPlacements(columns: [epochColumn], usableWidth: usableWidth, maximizedIndex: nil)
        ...
        expectEqual(ColumnLayoutEngine.clampProportion(0.1, minWidth: 800, maxWidth: nil, usableWidth: usableWidth),
                    floorProportion, "un piso conocido levanta la proporcion pedida")
        expectEqual(ColumnLayoutEngine.clampProportion(0.1, minWidth: nil, maxWidth: nil, usableWidth: usableWidth),
                    0.1, "sin piso conocido pasa tal cual, que es lo que hacia SIEMPRE")
        expectEqual(ColumnLayoutEngine.clampProportion(0.9, minWidth: nil, maxWidth: 400, usableWidth: usableWidth),
                    ColumnLayoutEngine.proportion(forWidth: 400, usableWidth: usableWidth),
                    "el techo de una regla recorta hacia abajo")
        expectEqual(ColumnLayoutEngine.clampProportion(0.5, minWidth: 800, maxWidth: 400, usableWidth: usableWidth),
                    floorProportion, "si techo y piso se cruzan, gana el piso: una ventana ilegible es peor que una ancha")
        expectEqual(ColumnLayoutEngine.clampProportion(2.0, minWidth: nil, maxWidth: nil, usableWidth: usableWidth),
        ...
        // haria.
        expect(TilingEngine.landsFloating(floatingLayerActive: true, isDialog: false, inFloatingList: true),
               "la que ya estaba flotando sigue flotando del otro lado")
        expect(TilingEngine.landsFloating(floatingLayerActive: false, isDialog: true, inFloatingList: false),
               "un dialog no se tilea aunque la capa activa sea la tileada")
        expect(!TilingEngine.landsFloating(floatingLayerActive: false, isDialog: false, inFloatingList: false),
        ...
        expect(cfg.centerFocusedColumn == NigiriConfig.CenterFocusedColumn.onOverflow, "center-focused-column on-overflow")
        ...
        """)
        expect(mixedPresets.presetColumnSizes == [.proportion(0.25), .fixed(1920), .proportion(0.75)],
        ...
        """)
        expect(heights.presetWindowHeightSizes == [.fixed(400), .proportion(0.5)],
               "preset-window-heights { fixed N } ya no se descarta")
        expect(!NigiriConfig.parse("layout { }").presetWindowHeightSizes.isEmpty,
        ...
        let heightThirds = (0..<3).map { _ in ColumnLayoutEngine.height(forProportion: 1.0 / 3.0, usableHeight: colH) }
        expectEqual(heightThirds.reduce(0, +) + 2 * ColumnLayoutEngine.gap, colH,
        ...
        expectEqual(NigiriConfig.tokenize("node /- arg1 arg2"), ["node", "arg2"],
                    "/- en medio de una linea se lleva UN argumento")
        expectEqual(NigiriConfig.tokenize("node arg /- { inner }"), ["node", "arg"],
        ...
        expectEqual(NigiriConfig.tokenize("a /* x /* y */ z */ b"), ["a", "b"], "comentarios de bloque anidados")

        // 5) escapes dentro de un string: la comilla escapada NO lo termina.
        expectEqual(NigiriConfig.tokenize("spawn \"say \\\"hola\\\"\""),
        ...
        expectEqual(NigiriConfig.tokenize("shader r\"a \\n b\""), ["shader", "a \\n b"],
                    "r\"...\" no interpreta escapes")
        expectEqual(NigiriConfig.tokenize("shader r#\"tiene \" adentro\"#"), ["shader", "tiene \" adentro"],
                    "r#\"...\"# aguanta comillas adentro")
        expectEqual(NigiriConfig.tokenize("shader #\"forma v2\"#"), ["shader", "forma v2"],
        ...
        let centerPlacements = ColumnLayoutEngine.columnPlacements(columns: centerCols, usableWidth: usableWidth, maximizedIndex: nil)
        ColumnLayoutEngine.centerPolicy = .never
        expectEqual(ColumnLayoutEngine.scrollOffset(toShow: 0, placements: centerPlacements, currentOffset: 0, usableWidth: usableWidth), 0,
                    "never: si ya se ve, no se mueve la camara")
        ColumnLayoutEngine.centerPolicy = .always
        let alwaysOffset = ColumnLayoutEngine.scrollOffset(toShow: 1, placements: centerPlacements, currentOffset: 0, usableWidth: usableWidth)
        expectEqual(alwaysOffset, centerPlacements[1].x + centerPlacements[1].width / 2 - usableWidth / 2, "always: centra la columna")
        ColumnLayoutEngine.centerPolicy = .onOverflow
        expectEqual(ColumnLayoutEngine.scrollOffset(toShow: 1, placements: centerPlacements, currentOffset: 0, usableWidth: usableWidth, previousIndex: 0), 0,
                    "on-overflow: si entran las dos, no centra")
        let farOffset = ColumnLayoutEngine.scrollOffset(toShow: 2, placements: centerPlacements, currentOffset: 0, usableWidth: usableWidth, previousIndex: 0)
        ...
        let singlePlacements = ColumnLayoutEngine.columnPlacements(columns: single, usableWidth: usableWidth, maximizedIndex: nil)
        expectEqual(ColumnLayoutEngine.scrollOffset(toShow: 0, placements: singlePlacements, currentOffset: 0, usableWidth: usableWidth),
                    singlePlacements[0].x + singlePlacements[0].width / 2 - usableWidth / 2, "always-center-single-column")
        ...
        expectEqual(NigiriConfig.mouseBindingKey(for: "Mod+MouseMiddle") ?? "", "mod-middle", "bind de boton del mouse")
        ...
        """)
        expectEqual(mouseCfg.mouseBindings["mod-middle"] ?? "", "close-window", "el bind de mouse no entra en binds{} de teclado")
        expect(mouseCfg.binds.contains { $0.combo == "F13" && $0.modifiers.isEmpty }, "el bind sin modificador se registra")
        ...
        if case .unknown = NiriProtocol.parse("{\"Nope\":{}}").request {} else { expect(false, "una peticion desconocida no se inventa") }
        expectEqual(NiriProtocol.kebab("MoveColumnToWorkspaceDown"), "move-column-to-workspace-down", "CamelCase -> kebab")

        // Los ids son estables y no se reciclan.
        let w1 = window(), w2 = window()
        expect(w1.id != w2.id, "cada ventana tiene su id")
        let wsA = Workspace(), wsB = Workspace()
        ...
        let colB = [CGRect(x: 720, y: 44, width: 700, height: 400),
                    CGRect(x: 720, y: 454, width: 700, height: 390)]
        let grid = [colA, colB]
        // Cerca del borde izquierdo de la primera: columna nueva antes.
        expect(ColumnLayoutEngine.insertPosition(columnFrames: grid, point: CGPoint(x: 12, y: 400), screenFrame: screen) == .newColumn(0),
        ...
        expect(ColumnLayoutEngine.insertPosition(columnFrames: grid, point: CGPoint(x: 1070, y: 449), screenFrame: screen) == .inColumn(1, 1),
               "sobre el hueco entre dos ventanas entra en la pila")
        // Empate: gana columna nueva (niri: `if vert_dist <= hor_dist`).
        let tie = ColumnLayoutEngine.insertPosition(columnFrames: grid, point: CGPoint(x: 715, y: 449), screenFrame: screen)
        ...
        expect(ColumnLayoutEngine.insertPosition(columnFrames: [], point: .zero, screenFrame: screen) == .newColumn(0),
        ...
        let overviewOut = ColumnLayoutEngine.overviewFrames(columns: overviewCols, in: screen, maximizedIndex: nil)
        expectEqual(overviewOut.count, 3, "las tres columnas entran al overview")
        expectEqual(overviewOut[2].frame.minX, screen.minX + 10 + 2 * (720 + 10), "la tercera conserva su x virtual, sin el clamp de pantalla")
        expect(overviewOut.allSatisfy { $0.frame.width > 2 && $0.frame.height > 2 },
        ...
        let clamped = ColumnLayoutEngine.targetFrames(columns: overviewCols, in: screen, maximizedIndex: nil, viewOffset: 0)
        expectEqual(clamped[2].frame.minX, screen.maxX - 1, "targetFrames si la clampea: por eso el overview no puede usarlo")
        ...
        let oneWs = OverviewPanel.computeRows([
            OverviewPanel.WorkspaceInput(wsIndex: 0, active: true, windows: [
        ...
        let pair = OverviewPanel.computeRows([
            ...
            OverviewPanel.WorkspaceInput(wsIndex: 1, active: false, windows: [
                (window: window(), layoutFrame: onScreen, captureFrame: onScreen)]),
        ], screenFrame: screen)
        expectEqual(pair.rows.count, 2, "dos workspaces ocupados, dos filas")
        expectEqual(pair.rows[1].canvas.minY - pair.rows[0].canvas.minY,
                    screen.height * 0.5 + screen.height * 0.1 * 0.5,
                    "el paso entre workspaces es alto + hueco del 10%")
        expectEqual(pair.rows[0].canvas.height, pair.rows[1].canvas.height,
                    "y todos miden lo mismo: el zoom es uno solo, no uno por fila")

        // REGRESSION (items 15 y 16): la regla de focus-follows-mouse.
        expect(!TilingEngine.shouldFocusFollowMouse(overviewActive: true, transitioning: false,
                                                    buttonsDown: 0, sinceLastTick: 1),
               "con el overview abierto NO mueve el foco: el panel maneja su propia seleccion")
        expect(!TilingEngine.shouldFocusFollowMouse(overviewActive: false, transitioning: true,
                                                    buttonsDown: 0, sinceLastTick: 1),
               "ni en medio de un cambio de workspace")
        expect(!TilingEngine.shouldFocusFollowMouse(overviewActive: false, transitioning: false,
        ...
        let previewFrames = ColumnLayoutEngine.targetFrames(columns: [tabbedDrag, plainDrag], in: screen,
        ...
        expect(previewFrames.allSatisfy { $0.frame.minX < screen.maxX - 100 },
               "y ninguna es el punto de parkeo del borde derecho")
        let withParked = ColumnLayoutEngine.targetFrames(columns: [tabbedDrag, plainDrag], in: screen,
                                                         maximizedIndex: nil, viewOffset: 0)
        expectEqual(withParked.count, 3, "el camino normal si las incluye, que es lo que las mantiene aparcadas")
        ...
        expectEqual(ColumnLayoutEngine.overviewFrames(columns: [tabbedCol], in: screen, maximizedIndex: nil).count, 1,
        ...
        expectEqual(NigiriConfig.parseColor("#f0a8")?.alphaComponent ?? 0, 0x88 / 255.0, "4 digitos con alpha")
        ...
        // inalcanzable).
        expectEqual(ColumnLayoutEngine.fitOffset(x: 100, width: 2000, currentOffset: 0, usableWidth: usableWidth), 100,
                    "una columna mas ancha que la vista se alinea a la izquierda")
        expectEqual(ColumnLayoutEngine.fitOffset(x: 100, width: 2000, currentOffset: 5000, usableWidth: usableWidth), 100,
        ...
        // movimiento cuesta.
        expectEqual(ColumnLayoutEngine.fitOffset(x: 20, width: 700, currentOffset: 0, usableWidth: usableWidth), 0,
                    "si ya entra, la camara se queda quieta")
        expectEqual(ColumnLayoutEngine.fitOffset(x: 1000, width: 700, currentOffset: 0, usableWidth: usableWidth),
                    1000 + 700 - usableWidth, "clipeada a la derecha: alinea ese borde")
        expectEqual(ColumnLayoutEngine.fitOffset(x: 300, width: 700, currentOffset: 900, usableWidth: usableWidth), 300,
        ...
        let halfPlacements = ColumnLayoutEngine.columnPlacements(columns: halves, usableWidth: usableWidth, maximizedIndex: nil)
        ...
        // la 1, y 1+2 entran.
        expectEqual(ColumnLayoutEngine.scrollOffset(toShow: 2, placements: halfPlacements, currentOffset: 0, usableWidth: usableWidth, previousIndex: 0),
        ...
        let widePlacements = ColumnLayoutEngine.columnPlacements(columns: wides, usableWidth: usableWidth, maximizedIndex: nil)
        expectEqual(ColumnLayoutEngine.scrollOffset(toShow: 1, placements: widePlacements, currentOffset: 0, usableWidth: usableWidth, previousIndex: 0),
        ...
        expectEqual(inc.removeWindows(where: { _ in false }).count, 0, "un removeWindows que no saca nada no toca el cache")
        ...
        let gone = twice.removeWindows { w in seen.append(w); return true }
        ...
        expect(TrackpadGestures.isMouseFamily(112), "family 112 es un Magic Mouse (lo reporta el hardware de esta maquina)")
